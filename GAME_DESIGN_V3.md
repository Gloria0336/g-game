...
起始裝備：
  upper: { id: 'covenant_coat',  durability: 75 }  契約者外衣
  lower: { id: 'covenant_skirt', durability: 75 }  契約者長裙
```
...
序章「契約書試煉」為純 VN 對話流程，玩家做出的選擇反映女主角面對未知的本能反應。系統靜默記錄分數，最後觸發覺醒演出——無介面式角色建立，一切發生在劇情中。

### 4.2 試煉問題與計分

**第一問：「妳看見前方有一個魔物正在逼近，妳第一個念頭是——」**
```
A. 「分析它的弱點，找準時機出手。」    → tactical +1
B. 「先確認自己的身體狀況和周圍環境。」 → spiritual +1
C. 「直接衝上去，想多了只會更慌。」    → instinct +1
```

**第二問：「有個受傷的陌生人倒在路邊，妳——」**
```
A. 「先觀察四周是否有陷阱，再決定要不要靠近。」 → tactical +1
B. 「感覺到一種說不清的衝動，想保護他。」       → spiritual +1
C. 「直接蹲下來，管那麼多幹嘛。」               → instinct +1
```

**第三問：「契約書告訴妳，召喚惡魔是有代價的。妳的態度是——」**
```
A. 「代價是什麼，我需要先弄清楚。」       → tactical +1
B. 「感覺……契約書在等待我，不只是恐懼。」 → spiritual +1
C. 「先用了再說，紙上談兵沒有意義。」     → instinct +1
```

**第四問：「幻境中出現了一扇門，妳不知道裡面是什麼。妳——」**
```
A. 「先繞到門後觀察門縫透出的資訊。」    → tactical +1
B. 「靜下來，感覺自己的直覺指向哪裡。」  → spiritual +1
C. 「推開它。」                          → instinct +1
```

### 4.3 覺醒判定與加成

| 最高分路徑 | 覺醒類型 | 數值加成 | 初始技能 |
|-----------|---------|---------|---------|
| tactical ≥ 2 | **戰術覺醒・洞察之眼** | AGI +4・insight +20 | 弱點標記（Tier I） |
| spiritual ≥ 2 | **靈性覺醒・意志之火** | WIL +4・maxSP +30 | 靈壓壓制（Tier I） |
| instinct ≥ 2 | **本能覺醒・獸性之刃** | ATK +6・maxHP +20 | 本能突刺（Tier I） |
| 三向平均 | **均衡覺醒・無名之力** | ATK+2・AGI+2・WIL+2 | 契約脈衝（Tier I，限定） |

### 4.4 初始技能規格

| 技能名 | 覺醒 | SP | 效果 |
|--------|------|-----|------|
| **弱點標記** | 戰術 | 14 | 本回合及下回合對目標傷害 +25%，附加移除目標 10% 迴避率 |
| **靈壓壓制** | 靈性 | 12 | 目標命中率 −20%（持續 2 回合），需 WIL ≥ 8 維持效果 |
| **本能突刺** | 本能 | 18 | 傷害倍率 ×1.5，忽略目標 10% DR |
| **契約脈衝** | 均衡 | 10 | 傷害倍率 ×1.2，同時回復自身 15 SP |
...
場景數：3
戰鬥：無（教學說明）
內容：世界觀導入 → 女主角獲得空白契約書 → 契約書試煉（覺醒系統）
```

#### 第一章「三個名字」
```
場景數：6
戰鬥：2 場（Tier A 魔物，教學戰鬥）
內容：三份契約書激活儀式 → 三名惡魔首次登場
特殊：三段各自的戰後對話，依女主角選擇各惡魔初始 heroine_axis ±10
技能：戰鬥掉落 Tier I
```
...
**各章節固定格式：**
```
[探索場景 × 1–2] → [戰鬥 × 1–2] → [惡魔對話 × 1] → [章節決策 × 1]
```

**情感里程碑：**
```
第 4 章末：contract_status 第一次判定（敵對 / 背叛初兆）
第 6 章末：主要路線分歧（傾心 vs 惡魔傾心 vs 敵對警告）
第 8 章末：不可逆轉點（路線鎖定）
```
...
| 路線 | 傾心（HE） | 惡魔傾心（HE/BE） | 敵對解決（NE） | 背叛（BE） |
|------|---------|--------------|------------|---------|
